...
export function FinalizeSaleModal({ isOpen, onClose, onFinalize, total }: Props) {
  ...
  const { discount, discountType, setDiscount, getSubtotal } = useCartStore();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Pagamento">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Lado esquerdo: entrada de valores e desconto */}
          ...
          <div className="p-4 border rounded-lg">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex bg-gray-200 rounded-lg p-1">
                <button
                  onClick={() => setDiscount(discount, 'R$')}
                  className={`px-3 py-1 text-sm font-bold rounded-md ${discountType === 'R$' ? 'bg-white shadow' : ''}`}
                >
                  R$
                </button>
                <button
                  onClick={() => setDiscount(discount, '%')}
                  className={`px-3 py-1 text-sm font-bold rounded-md ${discountType === '%' ? 'bg-white shadow' : ''}`}
                >
                  %
                </button>
              </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {paymentMethods.map(method => (
              <button
                ...
                className="p-4 border-2 border-pink-primary text-pink-primary rounded-lg font-bold hover:bg-pink-light/30"
              >
                {method}
              </button>
            ))}
        ...
        <div className="bg-gray-50 p-4 rounded-lg space-y-3 flex flex-col">
          ...
          {changeDue > 0 && (
            <div className="flex justify-between text-2xl font-bold text-blue-500 border-t pt-3">
              <span>TROCO:</span>
              <span>{changeDue.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
            </div>
          )}
          ...
          <button
            ...
            className="w-full mt-4 p-4 bg-pink-primary text-white rounded-lg font-bold text-xl disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Finalizar Venda
          </button>
        </div>
